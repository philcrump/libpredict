...
void sun_predict(predict_julian_date_t jul_time, double position[3])
{
	double mjd = jul_time - 2415020.0;
...
void predict_observe_sun(const predict_observer_t *observer, predict_julian_date_t jul_time, struct predict_observation *obs)
{
	// Find sun position
	double solar_vector[3];
	sun_predict(jul_time, solar_vector);
	...
	Calculate_Obs(jul_time, solar_vector, zero_vector, &geodetic, &solar_set);
 ...
 * \copyright GPLv2+
 **/
void predict_sun_ra_dec(predict_julian_date_t jul_time, double *ra, double *dec)
{
	...
	Calculate_RADec(jul_time, solar_vector, zero_vector, &geodetic, &solar_rad);
	*ra = solar_rad.x;
	*dec = solar_rad.y;
}
